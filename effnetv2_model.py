...
class SE(tf.keras.layers.Layer):
  ...
  def call(self, inputs):
    ...
    self.activations[self._se_reduce.name] = self._se_reduce(se_tensor)
    self.activations[self._se_reduce.name + "_after_act"] = self._act(self.activations[self._se_reduce.name])
    se_tensor = self._se_expand(self.activations[self._se_reduce.name + "_after_act"])

    self.activations[self._se_expand.name] = se_tensor
    self.activations[self._se_expand.name + "_after_act"] = tf.sigmoid(se_tensor)
    logging.info('Built SE %s : %s', self.name, se_tensor.shape)
    return self.activations[self._se_expand.name + "_after_act"] * inputs
...
class MBConvBlock(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training, survival_prob=None):
    ...
    x = inputs
    if self._block_args.expand_ratio != 1:
      self.activations[self._expand_conv.name] = self._expand_conv(x)
      x = self._act(self._norm0(self.activations[self._expand_conv.name], training=training))
      self.activations[self._expand_conv.name + "_after_act"] = x
      logging.info('Expand shape: %s', x.shape)

    self.activations[self._depthwise_conv.name] = self._depthwise_conv(x)
    x = self._act(self._norm1(self.activations[self._depthwise_conv.name], training=training))
    self.activations[self._depthwise_conv.name + "_after_act"] = x
    ...
    self.activations[self._project_conv.name] = self._project_conv(x)
    x = self._norm2(self.activations[self._project_conv.name], training=training)
    self.activations[self._project_conv.name + "_after_act"] = x
...
class FusedMBConvBlock(MBConvBlock):
  ...
  def call(self, inputs, training, survival_prob=None):
    ...
    self.activations[self._project_conv.name] = self._project_conv(x)
    x = self._norm1(self.activations[self._project_conv.name], training=training)
    if self._block_args.expand_ratio == 1:
...
class Stem(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training):
    self.activations[self._conv_stem.name] = self._conv_stem(inputs)
    out = self._act(self._norm(self.activations[self._conv_stem.name], training=training))
    self.activations[self._conv_stem.name + "_after_act"] = out
    return out
...
class Head(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training):
    """Call the layer."""
    self.activations[self._conv_head.name] = self._conv_head(inputs)
    outputs = self._act(self._norm(self.activations[self._conv_head.name], training=training))
    self.activations[self._conv_head.name+"_after_act"] = outputs
