...
def main(_) -> None:
  ...
  with ds_strategy.scope():
    ...
    distributed_ds = get_dataset(training=False, image_size=train_size, config=config)

    distributed_iterator = iter(distributed_ds)
    x_batch, y_batch = next(distributed_iterator)
    x_batch = x_batch['image'].numpy()
    y_batch = y_batch['label'].numpy()

    print("x_batch.shape", x_batch.shape)
    print("y_batch.shape", y_batch.shape)
    print(config)
    print("daaaata", config.data)

    layer_dict = {}
    for i in range(len(model.layers)):
      layer = model.layers[i]
      print("Main Model Layer", layer.name, "|", type(layer), "|", "effnetv2_model." in str(type(layer)))
      # print(vars(layer))
      if "effnetv2_model." in str(type(layer)):
        for inner_layer in layer._self_tracked_trackables:
          if hasattr(inner_layer, "name"):
            print("\tInner Layer", inner_layer.name, "|", type(inner_layer))
            layer_dict[inner_layer] = layer

    # - Defining the parameters of the initialization technique for ReLU layers
    weight_init_params = WeightInitScheme1Params(
      batch=x_batch,
    ...
    model = WeightInitScheme1.initialize_layers_effnet(model, layer_dict, weight_init_params)
    ...
    if FLAGS.mode == 'traineval':
      ...
      for ckpt in tf.train.checkpoints_iterator(
        ...
        try:
          current_epoch = int(os.path.basename(ckpt).xsplit('-')[1])
        except IndexError:
